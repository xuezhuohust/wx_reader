...
Page({
  ...
  onLoad(options) {
    ...
    this.syncHomeLayout("onLoad");
    this.bindHomeWindowResize();
  ...
  onResize(res) {
    this.syncHomeLayout("onResize", res && res.size);
    this.scheduleHomeLayoutSync();
    syncRoleTabBar(this, "pages/index/index");
  },

  syncHomeLayout(source, size) {
    const app = getApp();
    const layout = app.refreshLayout(`index:${source}`, size);
    app.setupNavBar();
    const menuButtonInfo = wx.getMenuButtonBoundingClientRect();
    const windowWidth =
      layout.width || (layout.windowInfo && layout.windowInfo.windowWidth) || 0;
    this.setData({
      navBarHeight: app.globalData.navBarHeight + 24,
      statusBarHeight: app.globalData.statusBarHeight,
      menuTop: menuButtonInfo.top,
      menuHeight: menuButtonInfo.height,
      menuRight: windowWidth - menuButtonInfo.left + 10,
      isLargeScreen: layout.isLandscapePad,
      isPadPortrait: layout.isPad && !layout.isLandscape,
      windowHeight: layout.height,
    });
  },

  scheduleHomeLayoutSync() {
    clearTimeout(this._homeLayoutSyncTimer);
    // 等系统窗口与胶囊按钮信息完成旋转后再读取一次，覆盖 Android 的过渡帧。
    this._homeLayoutSyncTimer = setTimeout(() => {
      this.syncHomeLayout("resizeSettled");
      syncRoleTabBar(this, "pages/index/index");
    }, 160);
  },

  bindHomeWindowResize() {
    if (
      typeof wx.onWindowResize !== "function" ||
      this._homeWindowResizeHandler
    ) {
      return;
    }
    this._homeWindowResizeHandler = (event) => {
      this.syncHomeLayout("windowResize", event && event.size);
      this.scheduleHomeLayoutSync();
    };
    wx.onWindowResize(this._homeWindowResizeHandler);
  },

  // App 级旋转事件用于补齐部分 Android Pad 不触发页面 onResize 的情况。
  handleAppLayoutChange(size) {
    this.syncHomeLayout("appLayoutChange", size);
    this.scheduleHomeLayoutSync();
    syncRoleTabBar(this, "pages/index/index");
    this.syncHomeTabBarVisibility();
  },

  syncHomeTabBarVisibility() {
    const tabBar = this.getTabBar && this.getTabBar();
    if (tabBar && typeof tabBar.setHidden === "function") {
      tabBar.setHidden(!!this.data.showPrivacyModal);
    }
  },
  ...
  onShow() {
    ...
    this.syncHomeLayout("onShow");

    syncRoleTabBar(this, "pages/index/index");
    // 旋转重建页面后，自定义 TabBar 的组件实例可能保留了之前弹层的隐藏
    // 状态。首页没有弹层时必须显式恢复，避免底部导航被永久隐藏。
    this.syncHomeTabBarVisibility();
    this.bootstrapHome();
  ...
  onUnload() {
    ...
    if (
      ...
      wx.offWindowResize(this._homeWindowResizeHandler);
      this._homeWindowResizeHandler = null;
    }
  },
  ...
  _distributeBooks(allBooks) {
    ...
    const enhancedAllBooks = allBooks.map((book, index) => {
      const realProgress = allProgress[book.id]
        ? allProgress[book.id].bookProgress
        : null;
      return Object.assign({}, book, {
        recommendReason:
          book.recommendReason || reasons[index % reasons.length],
        // 如果有真实进度则使用真实进度，否则使用模拟进度（为了演示美观，默认书籍给一点进度，新书为 0）
        progress: realProgress !== null ? realProgress : book.progress || 0,
        isAIReady:
          book.isAIReady !== undefined ? book.isAIReady : Math.random() > 0.4,
        rating: book.rating || (4 + Math.random()).toFixed(1),
        readersCount:
          book.readersCount || Math.floor(Math.random() * 5000) + 100,
        coverColor:
          book.coverColor ||
          ["#f3efe8", "#f5f7f4", "#e8f3f0", "#f3e8e8"][
  ...
  handleSearchClear() {
    this.setData({ searchValue: "" }, () => {
      this.applyFilters(this.data.books, "", this.data.activeCategory);
    });
  },
  ...
  switchRole(role, targetUrl) {
    ...
    if (getUserRole(currentIdentity) === role) {
      ...
      return this.loadRoleContent(currentIdentity).then(() => {
        if (targetUrl) {
    ...
    return switchActiveRole(role)
      .then((nextIdentity) => {
        ...
        this._enterMain(identity);
        return this.loadRoleContent(identity).then(() => {
          if (targetUrl) {
            wx.switchTab({ url: targetUrl });
          }
          return identity;
